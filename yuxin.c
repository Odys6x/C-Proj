...
void insertRecord(char data[MAX_RECORDS][MAX_LENGTH], int *recordCount, const char* key, const char* value) {
    ...
    for (int i = 0; i < (*recordCount) * 2; i += 2) {
        char datacpy[sizeof(data[i])];
        char keycpy[sizeof(key)];
        
        strcpy(datacpy, data[i]);
        strcpy(keycpy, key);

        if (strcmp(strlwr(datacpy), strlwr(keycpy)) == 0) {
...
void queryRecord(char data[MAX_RECORDS][MAX_LENGTH], char* keyword, int recordCount) {
    int found = 0;
...
void showAllRecords(char data[MAX_RECORDS][MAX_LENGTH], int* recordCount, char* filename) {
...
void updateRecord(char data[MAX_RECORDS][MAX_LENGTH], int recordCount, const char* key, const char* value) {
    const int count = recordCount;
